	...
	try {
		...
		const promises = files.map((fileNameWithExtension) => {
			...
			return limit(async () => {
				...
				const vaData = await generateVAP(sourceFolder, fileNameWithExtension);
				await saveData(vaData, fileNameWithExtension, destinationFolder);
			});
		});
...
// ---------------- RUN THE FUNCTION --------------- //
// ------------------------------------------------- //
processFilesInFolder("./data/upload", "./data/generated-VA-data");
